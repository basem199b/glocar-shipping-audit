...
export default function HomePage() {
  return (
    <main style={{ minHeight: '100vh', background: '#f5f7fb', color: '#142143', fontFamily: 'Arial, sans-serif', direction: 'rtl', display: 'flex' }}>
      <aside style={{ width: 280, background: '#142143', color: 'white', minHeight: '100vh', padding: 24, position: 'sticky', top: 0 }}>
        <div style={{ marginBottom: 34 }}>
          <h1 style={{ margin: 0, fontSize: 30, letterSpacing: 1 }}>GLO CAR</h1>
          <p style={{ margin: '8px 0 0', color: '#d8d0bd', fontSize: 13 }}>تدقيق فواتير الشحن</p>
        </div>

        <nav style={{ display: 'grid', gap: 10 }}>
          {menuItems.map((item) => (
            <button key={item.label} style={{ ...navButton, ...(item.active ? activeNavButton : {}) }}>
              <span>{item.icon}</span>
      ...
      <section style={{ flex: 1, padding: 30 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 20 }}>
          <div style={cardStyle}>
            <h3 style={{ marginTop: 0, fontSize: 24 }}>رفع ملف شحنات سلة</h3>
            <p style={{ color: '#667085', lineHeight: 1.8 }}>ارفع ملف Excel الصادر من سلة ويحتوي على رقم الطلب، رقم البوليصة، شركة الشحن، ومبلغ الشحن المحصل من العميل.</p>
        ...
        <div style={cardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div>
              <h3 style={{ margin: 0, fontSize: 24 }}>نتائج المطابقة</h3>
...
const cardStyle = {
  ...
  borderRadius: 18,
  padding: 22,
  boxShadow: '0 8px 24px rgba(20,33,67,.08)',
}
