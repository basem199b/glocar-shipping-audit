...
const initialCarriers: { id: number; name: string; invoices: number; status: string }[] = []
const initialInvoices: { id: number; carrier: string; month: string; shipments: number; amount: number; invoiceFile: string; paid: boolean; receipt: string; paidAt: string }[] = []
const sampleShipments: { carrier: string; waybill: string; source: string; order: string; amount: number; status: string }[] = []
...
function ReportsPage({ carriers, invoices }: { carriers: typeof initialCarriers; invoices: typeof initialInvoices }) {
  ...
  return <><PageHeader title="التقارير" desc="تقرير مستقل لكل شركة يوضح الفواتير، مبالغ سلة، عدد البوالص، والفرق بين سلة والفواتير الصادرة." />
    {rows.length === 0 ? <div style={emptyState}>لا توجد بيانات تقارير بعد. أضف شركات الشحن ثم ارفع بيانات سلة والفواتير.</div> : <div style={{ display: 'grid', gap: 18 }}>
...
function CarrierReportsPage({ carriers, invoices, attachReceipt, deleteReceipt }: { carriers: typeof initialCarriers; invoices: typeof initialInvoices; attachReceipt: (invoiceId: number, fileName: string) => void; deleteReceipt: (invoiceId: number) => void }) {
  ...
  const currentCarrier = selectedReportCarrier || carriers[0]?.name || ''
  const [reportView, setReportView] = useState<'shipments' | 'invoices'>('shipments')
  const selectedInvoices = invoices.filter((item) => item.carrier === currentCarrier)
  const selectedShipments = sampleShipments.filter((item) => item.carrier === currentCarrier)
  return <><PageHeader title="شركات الشحن" desc="تقارير فواتير وشحنات كل شركة شحن مضافة في النظام." />{carriers.length === 0 ? <div style={emptyState}>لا توجد شركات شحن بعد. أضف شركة شحن أولاً.</div> : <><div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 18 }}>{carriers.map((carrier) => <button key={carrier.id} onClick={() => setSelectedReportCarrier(carrier.name)} style={{ ...carrierButton, ...(currentCarrier === carrier.name ? activeCarrierButton : {}) }}>{carrier.name}</button>)}</div><div style={cardStyle}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 18 }}><div><h3 style={titleStyle}>تقرير {currentCarrier}</h3><p style={descStyle}>اختر عرض الفواتير أو قائمة الشحنات المسجلة من سلة ومن الفواتير.</p></div><div style={{ display: 'flex', gap: 10 }}><button onClick={() => setReportView('shipments')} style={{ ...tabButton, ...(reportView === 'shipments' ? activeTabButton : {}) }}>قائمة الشحنات</button><button onClick={() => setReportView('invoices')} style={{ ...tabButton, ...(reportView === 'invoices' ? activeTabButton : {}) }}>الفواتير</button></div></div>{reportView === 'shipments' && (selectedShipments.length === 0 ? <div style={emptyState}>لا توجد شحنات مسجلة لهذه الشركة.</div> : <table style={tableStyle}><thead><tr style={{ background: '#eef2f7' }}><th style={th}>رقم البوليصة</th><th style={th}>المصدر</th><th style={th}>رقم الطلب</th><th style={th}>المبلغ</th><th style={th}>الحالة</th></tr></thead><tbody>{selectedShipments.map((row) => <tr key={row.waybill} style={{ borderBottom: '1px solid #edf0f5' }}><td style={td}>{row.waybill}</td><td style={td}>{row.source}</td><td style={td}>{row.order}</td><td style={td}>{row.amount} ر.س</td><td style={td}><span style={row.status === 'مفوترة' ? greenBadge : yellowBadge}>{row.status}</span></td></tr>)}</tbody></table>)}{reportView === 'invoices' && (selectedInvoices.length === 0 ? <div style={emptyState}>لا توجد فواتير مسجلة لهذه الشركة.</div> : <table style={tableStyle}><thead><tr style={{ background: '#eef2f7' }}><th style={th}>شهر الفاتورة</th><th style={th}>عدد الشحنات</th><th style={th}>مبلغ الفاتورة</th><th style={th}>الفاتورة</th><th style={th}>حالة السداد</th><th style={th}>إيصال السداد</th><th style={th}>تاريخ السداد</th><th style={th}>تعديل الإيصال</th></tr></thead><tbody>{selectedInvoices.map((row) => <tr key={row.id} style={{ borderBottom: '1px solid #edf0f5' }}><td style={td}>{row.month}</td><td style={td}>{row.shipments}</td><td style={td}>{row.amount} ر.س</td><td style={td}><button onClick={() => openAttachment(row.invoiceFile)} style={viewButton}>رؤية الفاتورة</button></td><td style={td}><span style={row.paid ? greenBadge : yellowBadge}>{row.paid ? 'مسدد' : 'غير مسدد'}</span></td><td style={td}>{row.receipt ? <button onClick={() => openAttachment(row.receipt)} style={viewButton}>رؤية الإيصال</button> : <label style={smallUploadButton}>إرفاق إيصال<input hidden type="file" accept="image/*,application/pdf" onChange={(e) => { const file = e.target.files?.[0]; if (file) attachReceipt(row.id, file.name) }} /></label>}</td><td style={td}>{row.paidAt || '—'}</td><td style={td}>{row.receipt ? <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><button onClick={() => deleteReceipt(row.id)} style={dangerButton}>حذف الإيصال</button><label style={smallUploadButton}>إعادة إرفاق<input hidden type="file" accept="image/*,application/pdf" onChange={(e) => { const file = e.target.files?.[0]; if (file) attachReceipt(row.id, file.name) }} /></label></div> : '—'}</td></tr>)}</tbody></table>)}</div></>}</>
}
