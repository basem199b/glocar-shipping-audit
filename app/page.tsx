  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f4f6fb', direction: 'rtl', fontFamily: 'Arial', display: 'flex' }}>
      <aside style={{ ...sidebarStyle, width: sidebarOpen ? 270 : 86 }}>
        ...
        <div style={{ marginBottom: 34, overflow: 'hidden' }}><h1 style={{ margin: 0, fontSize: sidebarOpen ? 34 : 22, whiteSpace: 'nowrap' }}>GLO CAR</h1>{sidebarOpen && <p style={{ color: '#d8d0bd', marginTop: 8 }}>تدقيق فواتير الشحن</p>}</div>
        <nav style={{ display: 'grid', gap: 12 }}>{menuItems.map((item) => <button key={item.id} onClick={() => setActiveMenu(item.id)} style={{ ...navButton, ...(activeMenu === item.id ? activeButton : {}) }}><span>{item.icon}</span>{sidebarOpen && <span>{item.label}</span>}</button>)}</nav>
      </aside>
