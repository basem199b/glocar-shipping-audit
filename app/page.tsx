...
export default function HomePage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f4f6fb', direction: 'rtl', fontFamily: 'Arial', display: 'flex' }}>
      <aside style={{ ...sidebarStyle, width: sidebarOpen ? 260 : 72 }}>
        <button onClick={() => setSidebarOpen(!sidebarOpen)} style={collapseButton}>{sidebarOpen ? '×' : '☰'}</button>
        <div style={brandWrap}>
          <div style={logoCircle}>{sidebarOpen ? 'G' : 'G'}</div>
          {sidebarOpen && <div><h1 style={brandTitle}>GLO CAR</h1><p style={brandSub}>تدقيق فواتير الشحن</p></div>}
        </div>
        <nav style={{ display: 'grid', gap: 10 }}>{menuItems.map((item) => <button key={item.id} onClick={() => setActiveMenu(item.id)} title={item.label} style={{ ...navButton, justifyContent: sidebarOpen ? 'flex-start' : 'center', ...(activeMenu === item.id ? activeButton : {}) }}><span>{item.icon}</span>{sidebarOpen && <span>{item.label}</span>}</button>)}</nav>
      </aside>
...
const sidebarStyle: CSSProperties = { background: '#142143', color: 'white', minHeight: '100vh', padding: 14, transition: '.25s ease', position: 'sticky', top: 0, boxSizing: 'border-box' }
const collapseButton: CSSProperties = { width: 44, height: 44, borderRadius: '50%', background: '#0f1b39', border: '1px solid rgba(255,255,255,.16)', color: 'white', fontSize: 24, cursor: 'pointer', marginBottom: 22, display: 'grid', placeItems: 'center' }
const brandWrap: CSSProperties = { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 30, overflow: 'hidden' }
...
const navButton: CSSProperties = { width: '100%', border: 0, background: 'transparent', color: 'white', display: 'flex', alignItems: 'center', gap: 12, padding: '14px', borderRadius: 999, fontWeight: 700, cursor: 'pointer' }
